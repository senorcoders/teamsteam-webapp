...
export class ViewContactsComponent implements OnInit {
  ...
  public image = '';
  public contactsYahoo: {
    contacts: { contact: any[], count: number, start: number, total: number, uri: string }
  } = {contacts: {contact: [], count: 0, start: 0, total: 0, uri: ''}};
  ...
  constructor(
    ...
    for (const role of this.user.roles) {
      if (
        role.name === 'Manager' &&
        Object.prototype.toString.call(role.team) === '[object Object]'
      ) {
  ...
  private async getPlayers() {
    try {
      ...
      for (const id of this.teams) {
        ...
        players = players.filter(it => {
          return Object.prototype.toString.call(it.user) === '[object Object]'
        })
  ...
  private async initConnetionSockets() {
    try {
      ...
      const script = await this.http.get('/script/socket', {responseType: 'text'}).toPromise() as string;
      new Function(script)();
      const user = this.auth.getLoginData();
      io.sails.query = `user=${user.id}&token=${user.token}`;
      this.conexion = io.sails.connect(Interceptor.url, {reconnection: true});
  ...
  public validSameGoogle(ct) {
    ...
    if (ct.hasOwnProperty('phoneNumbers')) {
      for (const phone of ct.phoneNumbers) {
    ...
    if (ct.hasOwnProperty('emailAddresses')) {
      for (const email of ct.emailAddresses) {
  ...
  public async grantPermissionGoogle() {
    try {
      const link: any = await this.http.get(`/google/url-auth`).toPromise();
  ...
  public async grantPermissionYahoo() {
    try {
      const link: any = await this.http.get(`/yahoo/url-auth`).toPromise();
      window.location.href = link.url;
    } catch (e) {
      console.error(e);
    }
  ...
  public unselectContactAny(isGoogle, contact) {
    if (isGoogle) {
      if (this.selectsContactsGoogle.length > 1) {
        const index = this.selectsContactsGoogle.findIndex(it => {
      ...
      if (this.selectsContactsYahoo.length > 1) {
        const index = this.selectsContactsYahoo.findIndex(it => {
          return it.resourceName === contact.resourceName;
        });
        if (index !== -1) {
  ...
  public async importAsPlayer() {
    try {
      ...
      for (const contact of this.selectsContactsGoogle) {
        await this.import(true, contact);
      }
      this.selectsContactsGoogle = [];
      for (const contact of this.selectsContactsYahoo) {
  ...
  private setPorcentage() {
    const loadings = document.querySelectorAll('.loading-porcentage') as any;
    for (const load of (loadings as HTMLDivElement[])) {
