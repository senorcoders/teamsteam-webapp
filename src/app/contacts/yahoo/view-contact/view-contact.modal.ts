...
export class ViewContactYahoo {
    ...
    public edit() {
        this.router.navigate([`contacts/edit/yahoo/contact`], { queryParams: { uri: this.contact.uri } })
    ...
    public async deleteExecute() {
        ...
        try {
            await this.http.delete(`/yahoo/contacts/${this.userId}?uri=${this.contact.uri}`, { responseType: "text" }).toPromise();
